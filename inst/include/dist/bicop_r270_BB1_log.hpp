#ifndef VIFCOPULA_DISTRIBUTION_BICOP_R270_BB1_LOG_HPP
#define VIFCOPULA_DISTRIBUTION_BICOP_R270_BB1_LOG_HPP
...
using namespace stan;
    ...
    /**
     * The log of the bivariate rotated 270 degree BB1 copula density for the specified vector(s) u
    ...
    bicop_r270_BB1_log(const T_u& u, const T_v& v, const T_theta& theta, const T_delta& delta) {
      static const char* function("vifcopula::bicop_r270_BB1_log");
...
//      check_bounded(function, "dependence parameter", delta,0,1);
      ...
      for (size_t i = 0; i < length(theta); i++) {
        theta_value[i] = - value_of(theta_vec[i]); // negative params
        delta_value[i] = - value_of(delta_vec[i]); // negative params
      ...
      for (size_t n = 0; n < N; n++) {
        T_partials_return u_dbl = value_of(u_vec[n]);        // Rotate 90 degree
        T_partials_return v_dbl = 1. - value_of(v_vec[n]);        // No const
        ...
        if (include_summand<propto, T_u, T_v, T_theta, T_delta>::value)
         ...
         if (!is_constant_struct<T_u>::value){
           ...
           ops_partials.edge1_.partials_[n] +=  -(theta_value[n] + 1.) / u_dbl + \
             dx_du * ( - (1./thde + 2.*inv_delta[n]) * xpydm1 / (1. + xpyd) + \
                       ...
                       (1 - inv_delta[n]) / x_dbl ) ; // Negative rotation for u
         ...
         if (!is_constant_struct<T_theta>::value){
           double eps = - 0.001; // Negative calculation
           ...
           T_partials_return log_c_new = -(inv_theta_new + 2) * log(1 + xpyd_new) +\
            ...
            ops_partials.edge3_.partials_[n] -= (log_c_new - log_c)/eps; // Negative calculation
         }
         ...
         if (!is_constant_struct<T_delta>::value){
           ...
           T_partials_return log_c_new = -(inv_theta_new + 2) * log(1 + xpyd_new) +\
             ...
             (1 - inv_delta_new) * logxy_new -                               \
             (theta_new + 1) * (log(u_dbl) + log(v_dbl)) ;
           ops_partials.edge4_.partials_[n] -= (log_c_new - log_c)/eps; // Negative calculation
    ...
    typename stan::return_type<T_u, T_v, T_theta, T_delta>::type
    bicop_r270_BB1_log(const T_u& u, const T_v& v, const T_theta& theta, const T_delta& delta) {
      return bicop_r270_BB1_log<false>(u, v, theta, delta);
    }

}
#endif // VIFCOPULA_DISTRIBUTION_BICOP_R270_BB1_LOG_HPP
