#ifndef VIFCOPULA_DISTRIBUTION_BICOP_SURVIVAL_GUMBEL_LOG_HPP
#define VIFCOPULA_DISTRIBUTION_BICOP_SURVIVAL_GUMBEL_LOG_HPP
...
using namespace stan;
    ...
    /**
     * The log of the bivariate Survival Gumbel copula density for the specified vector(s) u
    ...
    bicop_survival_gumbel_log(const T_u& u, const T_v& v, const T_theta& theta) {
      static const char* function("vifcopula::bicop_survival_gumbel_log");
...
//      check_positive_finite(function, "dependence parameter", theta - 1); // theta >1
      ...
      for (size_t n = 0; n < N; n++) {
        const T_partials_return u_dbl = 1 - value_of(u_vec[n]); // survial
        const T_partials_return v_dbl = 1- value_of(v_vec[n]); // survial
    ...
    typename stan::return_type<T_u, T_v, T_theta>::type
    bicop_survival_gumbel_log(const T_u& u, const T_v& v, const T_theta& theta) {
      return bicop_survival_gumbel_log<false>(u, v, theta);
    }

}
#endif // VIFCOPULA_DISTRIBUTION_BICOP_SURVIVAL_GUMBEL_LOG_HPP
