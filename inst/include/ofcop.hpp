...
#include <onefcopulaLatent_stanc.hpp>
...
public:
    ...
    ofcopLatent (const matrix_d& u_,
            vector_d theta_,      //parameter1
            vector_d theta2_,     //parameter2
           std::vector<int>& copula_type_vec_,
           int t_max_,
           int n_max_,
           int k_,
           rng_t& base_rng_) :
    u(u_), theta(theta_), theta2(theta2_),copula_type_vec(copula_type_vec_),t_max(t_max_), n_max(n_max_), k(k_),
    base_rng(base_rng_)
    {
                ...
                int& count_select)
    {
        ...
        std::vector<int> gid(n_max);
        std::fill(gid.begin(), gid.end(), 0);
        onefcopulaLatent ObjOnefcopLatent(u,theta,theta2,gid,copula_type_vec,t_max, n_max, k, base_rng);


        // Dummy input
        Eigen::VectorXd cont_params = Eigen::VectorXd::Zero(ObjOnefcopLatent.num_params_r());

        // ADVI
        stan::variational::advi_mod<onefcopulaLatent, stan::variational::normal_meanfield, rng_t> advi_cop(ObjOnefcopLatent,
                                                                                                     cont_params,
                                                                                                     base_rng,
                                                                                                     n_monte_carlo_grad,
                                                                                                     n_monte_carlo_elbo,
                                                                                                     eval_elbo,
                                                                                                     iter);
        int max_param = ObjOnefcopLatent.num_params_r();
        sample_vi.resize(iter,max_param);
        mean_vi.resize(max_param);
        ...
        max_param = ObjOnefcopLatent.num_params_r();
        sample_vi.resize(iter,max_param);
        mean_vi.resize(max_param);
    ...
    )
    {
        ...
        std::vector<int> disc_vector;
        std::vector<double> cont_vector(ObjOnefcopLatent.num_params_r() ,0); // initial value at 0

        Eigen::VectorXd inv_metric;
        stan::io::dump dmp = stan::services::util::create_unit_e_diag_inv_metric(ObjOnefcopLatent.num_params_r());
        stan::io::var_context& unit_e_metric = dmp;

        try {
            inv_metric =
                stan::services::util::read_diag_inv_metric(unit_e_metric, ObjOnefcopLatent.num_params_r(),
                                                           logger);
            stan::services::util::validate_diag_inv_metric(inv_metric, logger);
        } catch (const std::domain_error& e) {
        }

        stan::mcmc::adapt_diag_e_nuts<onefcopulaLatent, boost::ecuyer1988>
            sampler(ObjOnefcopLatent, base_rng);
        ...
        stan::services::util::run_adaptive_sampler(sampler, ObjOnefcopLatent, cont_vector, num_warmup,
                                                   num_samples, num_thin, refresh, save_warmup,
                                                   base_rng, interrupt, logger,
                                                   sample_writer, diagnostic_writer);
