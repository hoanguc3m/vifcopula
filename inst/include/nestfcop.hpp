...
#include <nefcopulaLatent_stanc.hpp>
...
public:
    ...
    nestfcopLatent (const matrix_d& u_,
                ...
                int& count_select)
    {
        ...
        nefcopulaLatent ObjnestfcopLatent(u,theta, theta2,latent_theta, latent_theta2,
                                          gid,copula_type,latent_copula_type,
                              t_max, n_max, k, base_rng);


        // Dummy input
        Eigen::VectorXd cont_params = Eigen::VectorXd::Zero(ObjnestfcopLatent.num_params_r());

        // ADVI
        stan::variational::advi_mod<nefcopulaLatent, stan::variational::normal_meanfield, rng_t> advi_cop(ObjnestfcopLatent,
                                                                                                    cont_params,
                                                                                                    base_rng,
                                                                                                    n_monte_carlo_grad,
                                                                                                    n_monte_carlo_elbo,
                                                                                                    eval_elbo,
                                                                                                    iter);
        int max_param = ObjnestfcopLatent.num_params_r();
        sample_vi.resize(iter,max_param);
        mean_vi.resize(max_param);
        ...
        max_param = ObjnestfcopLatent.num_params_r();
        mean_vi.resize(max_param);
        sample_vi.resize(iter,max_param);
        advi_cop.write(vi_save, mean_vi, sample_vi, ELBO, modelselect, message_writer);
        out_parameter_writer.clear(); // Clear state flags.
    ...
    )
    {
        ...
        std::vector<int> disc_vector;
        std::vector<double> cont_vector(ObjnestfcopLatent.num_params_r() ,0); // initial value at 0

        Eigen::VectorXd inv_metric;
        stan::io::dump dmp = stan::services::util::create_unit_e_diag_inv_metric(ObjnestfcopLatent.num_params_r());
        stan::io::var_context& unit_e_metric = dmp;

        try {
            inv_metric =
                stan::services::util::read_diag_inv_metric(unit_e_metric, ObjnestfcopLatent.num_params_r(),
                                                           logger);
            stan::services::util::validate_diag_inv_metric(inv_metric, logger);
        } catch (const std::domain_error& e) {
        }

        stan::mcmc::adapt_diag_e_nuts<nefcopulaLatent, boost::ecuyer1988>
            sampler(ObjnestfcopLatent, base_rng);
        ...
        stan::services::util::run_adaptive_sampler(sampler, ObjnestfcopLatent, cont_vector, num_warmup,
                                                   num_samples, num_thin, refresh, save_warmup,
                                                   base_rng, interrupt, logger,
                                                   sample_writer, diagnostic_writer);
