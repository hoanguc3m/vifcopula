...
public:
    ...
    bool check_Ind(void){
        ...
        vector<double>* Uptr = u;
        vector<double>* Vptr = v;

        std::vector<int> order_U(t_max);
        // get order with compare_other
        for(int i = 0; i != order_U.size(); ++i)
          order_U[i] = i;
        sort(order_U.begin(), order_U.end(), compare_other<std::less<double>, double*>(Uptr));

        // reorder X and Y
        std::vector<double> U_ord(t_max);
        std::vector<double> V_ord(t_max);
        for(int i = 0; i != t_max; ++i)
          U_ord[i] = Uptr[order_U[i]];
        for(int i = 0; i != N; ++i)
          V_ord[i] = Vptr[order_U[i]];
        ...
        double tau = 0.02;//kendall_tau(concordant, discordant, extraX, extraY);
                 ...
                 std::ostream* pstream__ = 0) const
    {
        ...
        try
        {
            ...
            case 4:
                ...
                if (jacobian__)
                    theta = in__.scalar_lb_constrain(1,lp__);
                else
                    theta = in__.scalar_lb_constrain(1);
